// Calendario motivazionale v9
const DATA_KEY = "cm_data_v10";
const CONFIG_KEY = "cm_config_v3";
...
function clampCardioMinutes(v){
  const n = Number(v);
  return (n===30||n===60) ? n : 60;
}
function setCardioDurationUI(minutes){
  const wrap=document.getElementById("cardio-duration-wrap");
  if(!wrap) return;
  wrap.classList.remove("hidden");
  wrap.querySelectorAll(".seg-btn").forEach(b=>{
    b.classList.toggle("active", Number(b.dataset.min)===Number(minutes));
  });
}
function getCardioMinutesFromUI(){
  const wrap=document.getElementById("cardio-duration-wrap");
  if(!wrap) return 60;
  const active=wrap.querySelector(".seg-btn.active");
  return active ? clampCardioMinutes(active.dataset.min) : 60;
}
...
function ensureConfig(){
  ...
  // Normalizza (no vuoti, trim, no duplicati)
  const seen=new Set();
  config.workoutTypes = config.workoutTypes
    .map(x => String(x||"").trim())
    .filter(x => x.length>0)
    .filter(x => (seen.has(x)?false:(seen.add(x),true)));

  if(config.workoutTypes.length===0) config.workoutTypes=["Allenamento"];
...
function getDayData(date){
  ...
  if(!dataStore[iso]){
    dataStore[iso]={
      cardio:false, cardioMinutes:60, cardioReason:"",
...
function renderWorkoutSelect(){
  ...
  // Se possibile ripristina scelta precedente
  if(workoutOptions().includes(current)) sel.value=current;
}

function renderWorkoutChips(){
  const wrap=document.getElementById("workout-types-list");
  if(!wrap) return;
  wrap.innerHTML="";
  config.workoutTypes.forEach((t)=>{
    ...
    del.type="button";
    del.textContent="×";
    del.setAttribute("aria-label", `Elimina ${t}`);
    del.addEventListener("click", ()=>{
      // rimuovi da config
      config.workoutTypes = config.workoutTypes.filter(x=>x!==t);
      // se restano vuote, rimetti placeholder
      if(config.workoutTypes.length===0) config.workoutTypes=["Allenamento"];
      saveJSON(CONFIG_KEY, config);

      // Se il giorno corrente aveva quel workout selezionato, riportalo a Rest
      const day=getDayData(currentDate);
      if(day.workout===t){
...
function loadDayToUI(date){
  ...
  // Cardio durata (30' / 60')
  d.cardioMinutes = clampCardioMinutes(d.cardioMinutes ?? 60);
  const durWrap=document.getElementById("cardio-duration-wrap");
  if(durWrap){
    if(d.cardio){
      setCardioDurationUI(d.cardioMinutes);
    }else{
  ...
  // Se l'opzione non esiste più, fallback
  if(!workoutOptions().includes(d.workout)) d.workout=config.restLabel;
...
function saveCurrentDay(playSound){
  ...
  const next={
    ...
    cardioMinutes: document.getElementById("cardio-checkbox").checked ? getCardioMinutesFromUI() : 60,
...
function computeStatsForRange(start,end){
  ...
  Object.keys(dataStore).forEach(key=>{
    ...
    if(day.cardio){ const mins=clampCardioMinutes(day.cardioMinutes ?? 60); cardioDone += (mins/60); } else if(day.cardioReason){ cardioMiss++; cardioReasons[day.cardioReason]=(cardioReasons[day.cardioReason]||0)+1; }
...
function updateStatsMode(mode){
  ...
  setText("stats-cardio-value", `${fmtCount(s.cardioDone)} / ${cardioTarget}`);
...
function updateRangeStats(){
  ...
  const s=computeStatsForRange(start,end);
  setText("range-cardio-count", fmtCount(s.cardioDone));
...
(function init(){
  ...
  document.getElementById("btn-open-calendar").addEventListener("click", ()=>{ showScreen("screen-main"); loadDayToUI(currentDate); updateAllStats(); });
  ...
  document.getElementById("btn-add-workout-type").addEventListener("click", ()=>{
    ...
    if(!v) return;
    if(v===config.restLabel) return;
    if(config.workoutTypes.includes(v)) return;
  ...
  document.getElementById("cardio-checkbox").addEventListener("change", ()=>{
    const cb=document.getElementById("cardio-checkbox");
    const wrap=document.getElementById("cardio-duration-wrap");
    if(cb.checked){
      document.getElementById("cardio-reason-select").value="";
      if(wrap){ setCardioDurationUI(60); }
    }else{
  ...
  // Cardio durata buttons
  const durWrap=document.getElementById("cardio-duration-wrap");
  if(durWrap){
    durWrap.querySelectorAll(".seg-btn").forEach(btn=>{
      btn.addEventListener("click", ()=>{
        durWrap.querySelectorAll(".seg-btn").forEach(b=>b.classList.remove("active"));
        btn.classList.add("active");
        if(document.getElementById("cardio-checkbox").checked){
  ...
  // Setup screen initial chips render (first load)
  renderWorkoutChips();
})();
