...
// =========================
// Sistema motivazionale (badge, frasi, suoni)
// =========================

const MOTIVATION_MESSAGES = {
    cardio: [
        "Cardio fatto. Stai costruendo resistenza ogni giorno.",
        "Un passo in più verso la tua versione migliore.",
        "Il fiato migliora, la disciplina cresce.",
        "Hai fatto ciò che molti saltano. Bravo.",
        "Cardio completato: il futuro te ne sarà grato."
    ],
    workout: [
        "Allenamento completato. Potenza pura.",
        "Hai trasformato fatica in progresso reale.",
        "Ogni serie che fai ti rende più solido.",
        "Hai fatto ciò che andava fatto, anche senza voglia.",
        "Allenamento chiuso. Uno in meno tra te e il risultato."
    ],
    diet: [
        "Dieta rispettata. Disciplina vera.",
        "Hai vinto una battaglia importante oggi.",
        "Ogni scelta a tavola si vede sul lungo periodo.",
        "Continua così, stai costruendo nuove abitudini.",
        "Ottimo autocontrollo. Questo fa la differenza."
    ],
    weightLoss: [
        "Peso in calo: i sacrifici stanno pagando.",
        "Un passo concreto verso il tuo obiettivo.",
        "Si vede che stai facendo sul serio.",
        "Ogni etto in meno è lavoro fatto bene.",
        "Stai trasformando le intenzioni in risultati."
    ],
    weightUp: [
        "Il peso è salito un po'. Non è un problema se reagisci subito.",
        "Piccolo aumento: usalo come carburante per fare meglio da oggi.",
        "Può capitare. Riprendi il controllo con dieta e cardio.",
        "Nessun dramma: registra, accetta e torna al piano."
    ],
    weightSame: [
        "Peso stabile. Se vuoi dimagrire, tieni alta l'attenzione a dieta e movimento.",
        "Stesso peso di ieri: la costanza paga, ma serve ancora un po' di spinta.",
        "Se l'obiettivo è dimagrire, prova a curare qualche dettaglio in più oggi.",
        "Stabilità oggi, risultati domani se continui a fare le scelte giuste."
    ],
    achievement: [
        "Achievement unlocked: settimana completata.",
        "Hai rispettato gli impegni con te stesso.",
        "Settimana da atleta. Continua così.",
        "Hai dimostrato a te stesso che puoi farcela.",
        "Risultato importante. Non fermarti proprio ora."
    ]
};
...
function playMotivationSound(kind) {
    // kind: cardio, workout, diet, weight, achievement
    switch (kind) {
        case "cardio":
        case "diet":
        ...
        case "workout":
        case "weight":
            playTone(660, 0.22, 0.22);
            playTone(990, 0.22, 0.18);
            break;
        case "achievement":
            playTone(523, 0.2, 0.22);
            setTimeout(() => playTone(659, 0.2, 0.22), 140);
            setTimeout(() => playTone(784, 0.25, 0.22), 280);
            break;
        default:
            playTone(880, 0.18, 0.18);
    }
...
function showMotivationToast(kind) {
    let toast = document.getElementById("motivation-toast");
    if (!toast) {
    ...
    if (kind === "cardio" || kind === "diet") {
        toast.classList.add("motivation-toast--green");
    } else if (kind === "workout" || kind === "weight") {
        toast.classList.add("motivation-toast--blue");
    } else if (kind === "achievement") {
        toast.classList.add("motivation-toast--purple");
    }

    const list = MOTIVATION_MESSAGES[kind] || [];
    const msg = list.length ? list[Math.floor(Math.random() * list.length)] : "Ben fatto.";
...
function showCenterBadge(title, kind) {
    let badge = document.getElementById("motivation-badge-center");
    if (!badge) {
        badge = document.createElement("div");
        badge.id = "motivation-badge-center";
        badge.className = "motivation-badge-center";
        badge.innerHTML = '<div class="motivation-badge-title"></div><div class="motivation-badge-text"></div>';
        document.body.appendChild(badge);
    }
    const titleEl = badge.querySelector(".motivation-badge-title");
    const textEl = badge.querySelector(".motivation-badge-text");

    const list = MOTIVATION_MESSAGES[kind] || [];
    const msg = list.length ? list[Math.floor(Math.random() * list.length)] : "";
...
function loadDayToUI(date) {
    ...
    const weekday = formatItalianWeekday(currentDate);
    currentDayLabel.textContent = weekday.charAt(0).toUpperCase() + weekday.slice(1);
...
function saveCurrentDay(playSound) {
    ...
    lastWeightValue = prevData.weight != null && !isNaN(prevData.weight) ? Number(prevData.weight) : null;
    ...
    if (playSound && !prevComplete && nowComplete) {
        playMotivationSound("achievement");
        showCenterBadge("Giornata completata", "achievement");
    }
}
...
function updateWeightUI() {
    ...
    const weightBar = document.getElementById("weight-progress-bar");
    if (weightBar) {
        weightBar.style.width = percent + "%";
    }

    // Motivazione peso
    if (start != null && goal != null && w != null && !isNaN(w) && lastWeightValue != null && !isNaN(lastWeightValue)) {
        let changeType = null;

        if (goal < start) {
            // obiettivo dimagrimento
            if (w < lastWeightValue) {
                changeType = "weightLoss";
            } else if (w > lastWeightValue) {
                changeType = "weightUp";
            } else {
                changeType = "weightSame";
            }
        } else if (goal > start) {
            // obiettivo aumento peso
            if (w > lastWeightValue) {
                changeType = "weightLoss";
            } else if (w < lastWeightValue) {
                changeType = "weightUp";
        ...
        if (changeType === "weightLoss") {
            playMotivationSound("weight");
            showMotivationToast("weightLoss");
        } else if (changeType === "weightUp") {
            showMotivationToast("weightUp");
        } else if (changeType === "weightSame") {
            showMotivationToast("weightSame");
        }
    }

    lastWeightPercent = percent;
    lastWeightValue = w;
}
...
cardioCheckbox.addEventListener("change", () => {
    if (cardioCheckbox.checked) {
        ...
        playMotivationSound("cardio");
        showMotivationToast("cardio");
    }
    saveCurrentDay(true);
});
...
workoutSelect.addEventListener("change", () => {
    if (isGymWorkout(workoutSelect.value)) {
        workoutReasonSelect.value = "";
        playMotivationSound("workout");
        showMotivationToast("workout");
    }
    saveCurrentDay(true);
});
...
dietCheckbox.addEventListener("change", () => {
    if (dietCheckbox.checked) {
        ...
        playMotivationSound("diet");
        showMotivationToast("diet");
    }
    saveCurrentDay(true);
});
