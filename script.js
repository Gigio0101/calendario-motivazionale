...
// ---- Patch UI data iOS: pulsante data centrato ----
...
function formatItalianDatePill(date){
  try{
    return new Intl.DateTimeFormat("it-IT",{day:"numeric",month:"short",year:"numeric"}).format(date);
  }catch(e){
    return makeIsoLocal(date);
  }
}

// Override/extend: update text whenever day is loaded
const __origLoadDayToUI = loadDayToUI;
loadDayToUI = function(date){
  __origLoadDayToUI(date);
  if(dateDisplayText){
    dateDisplayText.textContent = formatItalianDatePill(currentDate);
  }
};

// Open picker on click
if(dateDisplayBtn && currentDateInput){
  dateDisplayBtn.addEventListener("click", ()=>{
    try{
      if(currentDateInput.showPicker) currentDateInput.showPicker();
      else currentDateInput.click();
    }catch(e){
      try{ currentDateInput.click(); }catch(_){}
    }
  });
}
