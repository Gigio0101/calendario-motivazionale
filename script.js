// =============================
//  MOTIVATION TRACKER PREMIUM (FIX + OGGI + IMPEDENZA COMPLETA)
// =============================

const STORAGE_KEY = "motivationTracker_premium_v8";
...
function ensureDay(dayId) {
    if (!DB.days[dayId]) {
        ...
        const e = DB.days[dayId];
        if (e.fatPercent === undefined) e.fatPercent = null;
        if (e.leanMassKg === undefined) e.leanMassKg = null;
        if (e.leanMassPercent === undefined) e.leanMassPercent = null;
    }
...
function renderDay() {
    ...
    dayCardEl.innerHTML =
        ...
        ' <div>' +
        '  <label for="weight" style="font-size:11px;">Peso (kg)</label>' +
        '  <input type="number" id="weight" step="0.1" value="' + (entry.weight != null ? entry.weight : "") + '" placeholder="Es. 82.5">' +
        " </div>" +
        ' <div>' +
        '  <label for="fat-percent" style="font-size:11px;">Massa grassa (%)</label>' +
        '  <input type="number" id="fat-percent" step="0.1" value="' + (entry.fatPercent != null ? entry.fatPercent : "") + '" placeholder="Es. 18">' +
        " </div>" +
        ' <div>' +
        '  <label for="lean-kg" style="font-size:11px;">Massa magra (kg)</label>' +
        '  <input type="number" id="lean-kg" step="0.1" value="' + (entry.leanMassKg != null ? entry.leanMassKg : "") + '" placeholder="Es. 65">' +
        " </div>" +
        ' <div>' +
        '  <label for="lean-percent" style="font-size:11px;">Massa magra (%)</label>' +
        '  <input type="number" id="lean-percent" step="0.1" value="' + (entry.leanMassPercent != null ? entry.leanMassPercent : "") + '" placeholder="Es. 82">' +
        " </div>" +
        "</div>" +

        '<div class="section-label">Note</div>' +
        '<textarea id="notes" placeholder="Appunti su come è andata la giornata, sensazioni, focus...">' +
    ...
    var fatPercentEl = document.getElementById("fat-percent");
    var leanKgEl = document.getElementById("lean-kg");
    var leanPercentEl = document.getElementById("lean-percent");
    ...
    function saveAndUpdate(source) {
        ...
        var fatVal = parseFloat(fatPercentEl.value);
        e.fatPercent = isNaN(fatVal) ? null : fatVal;

        var leanKgVal = parseFloat(leanKgEl.value);
        e.leanMassKg = isNaN(leanKgVal) ? null : leanKgVal;

        var leanPercVal = parseFloat(leanPercentEl.value);
        e.leanMassPercent = isNaN(leanPercVal) ? null : leanPercVal;
        ...
        // impedenza registrata se almeno un campo è compilato
        if ((e.fatPercent != null) || (e.leanMassKg != null) || (e.leanMassPercent != null)) {
    ...
    fatPercentEl.addEventListener("change", function () { saveAndUpdate("fat"); });
    leanKgEl.addEventListener("change", function () { saveAndUpdate("leanKg"); });
    leanPercentEl.addEventListener("change", function () { saveAndUpdate("leanPercent"); });
